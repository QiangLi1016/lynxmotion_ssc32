	...
	nh( nh )
{
	...
	{
		//ROS_INFO("In the if statement: of joints checker ");
		...
		for( joints_it = joints_struct.begin( ); joints_it != joints_struct.end( ); joints_it++ )
		{
			...
			//ROS_INFO("joint_graph_name for max angle: %s", (joint_graph_name + "max_angle").c_str());
	...
	if( priv_nh.getParam( "/controllers", controllers_list ) )
	{
		...
		//ROS_INFO("In the if statement of controllers checker ");
		...
		for( controllers_it = controllers_struct.begin( ); controllers_it != controllers_struct.end( ); controllers_it++ )
		{
			...
			// Is able to find the correct value from the parameter server
			...
			if( controller_type == "joint_controller" )
			{
				...
				//ROS_INFO("Retrieved proper joint controller type ");
			}
			...
			if( priv_nh.getParam( controller_graph_name + "joints", joints_list ) )
			{
				...
				for( unsigned int i = 0; i < joints_array.size( ); i++ )
				{
					...
					// Gets the correct joint names, even if more than one
			...
			{
				std::cout << "controller graph name: " << controller_graph_name;
